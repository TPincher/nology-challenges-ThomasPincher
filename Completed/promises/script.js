...
//   `delayedIncDec` returns a promise that resolves to the initial number incremented or decremented - think about what might cause this promise to reject
...
const delayedIncDec = (n, mode, timeout = 2500) => {
    return new Promise((resolve, reject) => {
        ...
        if(timeout < 0) {
            reject (newError('Timeout must be positive'))
        }
...
delayedIncDec(15, "dec", 200).then((result) => console.log(result));
...
delayedFilter([1,2,3,4,5], isEven, 500).then(result => console.log(result));