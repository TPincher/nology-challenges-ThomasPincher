...
decrement() {
  return this.count == 0 ? this.count = 0 : this.count -= 1;
}
}
...
export class Engine {
  ...
  startEngine() {
    ...
    } else this.engineIsRunning = true
  ...
  stopEngine() {
    ...
    } else this.engineIsRunning = false
...
export class BankAccount {
  ...
  deposit (dptAmount) {
    if (dptAmount > 0) {
      this._balance += Number(dptAmount)
      return this._balance
    } else return "Invalid input, unable to deposit"
  } 
  ...
  withdraw (wdlAmount) {
    if (this._balance < wdlAmount) {
      return "Insufficient funds, unable to withdraw"
    }
    if (wdlAmount >= 0) {
      this._balance -= Number(wdlAmount)
      return this._balance
    } else return "Invalid input, unable to withdraw"
  }
}
