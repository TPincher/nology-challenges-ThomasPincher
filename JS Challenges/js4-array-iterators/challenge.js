...
export const removeFalseValues = (booleanArr) => {
  let filteredArray = booleanArr.filter((value) => value == true);
...
export const createListOfPoessessions = (possessionsArr, name) => {
  let modifiedList = []
  possessionsArr.forEach((addTerm) => {
    modifiedList.push(name + " " + addTerm)
  })
  return modifiedList;
};
...
export const convertStringToNumbersArray = (numberString) => {
  ...
  finalString.push(Number(stringArray[i]))
  }
  return finalString
};
...
export const createOddEvenArray = (numberString) => {
  ...
  for (let i=0; i<stringArray.length; i++) {
    if (stringArray[i] % 2 == 0) { 
      finalString.push("even");
    } else {
      finalString.push("odd");
    }
  }
  return finalString
};
...
export const filterBooksBySearch = (booksArr, searchTerm) => {
  ...
  let currentBook = booksArr[i];
  if (currentBook.includes(searchTerm)) {
    matches.push(booksArr[i])
    }
  }
  return matches;
};
...
export const formatString = (string) => {
  ...
  //   if (regex.test(string[i]) && i%2==0) {
  //     finishedArray.push(string[i].toUpperCase())
  //   } else if (regex.test(string[i]) && i%2==1) {
  //     finishedArray.push(string[i].toLowerCase())
  //   }
  // }
  ...
  for (let i=0; i<string.length; i++) {
    if (regex.test(string[i])) {
      cleanedArray.push(string[i].toUpperCase())
    }
  }
  let finishedArray = []
    for (let n=0; n<cleanedArray.length; n++) {
      if (n % 2 == 0) {
        finishedArray.push(cleanedArray[n])
      } else {
        finishedArray.push(cleanedArray[n].toLowerCase())
      }
    }
