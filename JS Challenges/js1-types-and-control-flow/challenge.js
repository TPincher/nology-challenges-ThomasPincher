...
export const findType = () => {
  let varType = (typeof thing)
  if (varType == "string" || varType == "number" || varType == "boolean") {
    return "This is a " + (varType);
  } else {
...
export const getHasUppercaseLetters = () => {
    return /[A-Z]/.test(getHasUppercaseLetters)
};
...
export const convertPascalCaseToSnakeCase = () => {
  let newstring = ""
  newstring += pascalCaseVariableName[0].toLowerCase()
  for (let i = 1; i < pascalCaseVariableName.length; i++) {
    if (pascalCaseVariableName[i] == pascalCaseVariableName[i].toLowerCase()) {
